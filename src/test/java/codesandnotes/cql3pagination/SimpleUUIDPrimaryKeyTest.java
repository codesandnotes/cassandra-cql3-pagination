...
public class SimpleUUIDPrimaryKeyTest extends Integration {
    ...
    public void retrieve_5_ResultsOfPage_2_InAListOf_10_UsingCQL() {
        ...
        UUID lastIdInPage1 = page1.get(4).getUUID("id");

        List<Row> page2 = embeddedCassandra.session().execute(
                "select * from cql3_pagination.simple_uuid_pk where token(id) > token(" + lastIdInPage1 + ")"
        ).all();
        print("page 2:", page2);
